...
def printCSV(profs):
    ...
    # Imprima o resto do arquivo CSV aqui. Esta rotina devera retornar
    ...
    plantoes = buildTurns(profs)

    for (dia, periodo, profissional, indice) in plantoes:
...
def firstDay(profs, prof):
    ...
    for (dia, periodo, nome, indice) in plantoes:
        if nome == prof:
            found = False
            return dia
